...
| Property | Meaning |
|---|---|
| `Name` | Leaf name of the item |
| `FullName` | Absolute path |
| `Extension` | File extension, such as `.txt` |
| `Length` | File size in bytes |
| `LastWriteTime` | Last modification time |
| `Parent` | Parent directory object |
| `PSIsContainer` | `$true` for a directory, `$false` for a file |
...
1. State whether the input is a path string, an item object, or file content.
2. Build the exact path with `Join-Path`.
3. Validate existence and expected type with `Test-Path`.
4. Retrieve metadata with `Get-Item` or enumerate with `Get-ChildItem`.
5. Read or mutate only what the requirement needs.
6. Use `-WhatIf` before deletion.
7. Verify the resulting state.
8. Return objects when another command may consume the result.
